...
class LaporanController extends Controller
{
    ...
    public function indexLaporanBahanLoss(Request $request)
    {
        $user = Auth::user();
        $role = $user->role;
        ...
        $dateParam = $request->input('date'); // format 'YYYY-MM'
        ...
        if ($dateParam) {
            try {
                ...
                // BAR
                $allHistoriesBar = $this->generateAllHistories($selectedMonth, $selectedYear, 'BAR');
                foreach ($allHistoriesBar as $item) {
                    $total = collect($item['history'])->pluck('terbuang')->filter()->sum();
                    $totalM = collect($item['history'])->pluck('masuk')->filter()->sum();
                    $totalT = collect($item['history'])->pluck('terpakai')->filter()->sum();
                    $totalTerbuangBar[] = [
                        'name' => $item['bahan']->name,
                        'total_masuk' => $totalM,
                        'total_terpakai' => $totalT,
                        'total_terbuang' => $total,
                    ];
                }
                // KITCHEN
                $allHistoriesKitchen = $this->generateAllHistories($selectedMonth, $selectedYear, 'KITCHEN');
                foreach ($allHistoriesKitchen as $item) {
                    $total = collect($item['history'])->pluck('terbuang')->filter()->sum();
                    $totalM = collect($item['history'])->pluck('masuk')->filter()->sum();
                    $totalT = collect($item['history'])->pluck('terpakai')->filter()->sum();
                    $totalTerbuangKitchen[] = [
        ...
        return view('laporan.indexLaporanBahanLoss', compact(
            'dateParam',
            'bulanNama',
            'tahunNama',
            'totalTerbuangBar',
